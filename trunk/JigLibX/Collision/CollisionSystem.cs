...
namespace JigLibX.Collision
{
    ...
    public abstract class CollisionFunctor
    {
        ...
        public abstract void CollisionNotify(ref CollDetectInfo collDetectInfo,
            ref Vector3 dirToBody0, List<CollPointInfo> pointInfos);
    }
    ...
    public abstract class CollisionSystem
    {
        ...
        private Dictionary<KeyValuePair<int,int>,DetectFunctor> detectionFunctors = new Dictionary<KeyValuePair<int,int>,DetectFunctor>();
        ...
        {
            KeyValuePair<int,int> kp1 = new KeyValuePair<int,int>(f.Type0,f.Type1);
            KeyValuePair<int,int> kp2 = new KeyValuePair<int,int>(f.Type1,f.Type0);

            if (!detectionFunctors.ContainsKey(kp1))
                detectionFunctors.Add(kp1,f);

            if (!detectionFunctors.ContainsKey(kp2))
                detectionFunctors.Add(kp2,f);
        }
        ...
        public DetectFunctor GetCollDetectFunctor(int type0, int type1)
        {
            ...
            if (detectionFunctors.TryGetValue(kp, out functor))
                return functor;
            else
                return null;
        }
