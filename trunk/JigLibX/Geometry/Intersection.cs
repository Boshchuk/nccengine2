...
namespace JigLibX.Geometry
{
    public sealed class Intersection
    {
        ...
        public static bool SweptSpherePlaneIntersection(out Vector3 pt, out float finalPenetration, Sphere oldSphere, Sphere newSphere,
            Plane plane, float pOldDistToPlane, float pNewDistToPlane)
        {
            ...
            pt = oldSphere.Position + t * (newSphere.Position - oldSphere.Position) - MathHelper.Min(radius, oldDistToPlane) * plane.Normal;
        ...
        public static bool SweptSphereTriangleIntersection(out Vector3 pt, out Vector3 N, out float depth,
            Sphere oldSphere, Sphere newSphere, Triangle triangle,
            ...
            EdgesToTest edgesToTest, CornersToTest cornersToTest)
        {
            ...
            // Check against plane
            if (!SweptSpherePlaneIntersection(out pt,out depth, oldSphere, newSphere, trianglePlane, oldCentreDistToPlane, newCentreDistToPlane))
                return false;

            Vector3[] trianglePts = new Vector3[3] { triangle.GetPoint(0), triangle.GetPoint(1), triangle.GetPoint(2) };
            Vector3[] triangleEdges = new Vector3[3] { trianglePts[1] - trianglePts[0], trianglePts[2] - trianglePts[1], trianglePts[0] - trianglePts[2] };

            // If the point is inside the triangle, this is a hit
            bool allInside = true;
            for (i = 0; i < 3; ++i)
            {
                Vector3 outDir = Vector3.Cross(triangleEdges[i], trianglePlane.Normal);
                if (Vector3.Dot(pt - trianglePts[i], outDir) > 0.0f)
                {
                    allInside = false;
                    break;
                }
            ...
            Vector3 Ks = newSphere.Position - oldSphere.Position;
            ...
            for (i = 0; i < 3; ++i)
            {
                ...
                if (!((mask !=0) & ((int)edgesToTest != 0))) // TODO: CHECK THIS
                    continue;
                Vector3 Ke = triangleEdges[i];
                Vector3 Kg = trianglePts[i] - oldSphere.Position;
                ...
                Vector3 Ct = oldSphere.Position + t * Ks;
                float d = Vector3.Dot((Ct - trianglePts[i]), triangleEdges[i]) / kee;
