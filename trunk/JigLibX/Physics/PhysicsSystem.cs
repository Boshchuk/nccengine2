...
namespace JigLibX.Physics
{
    ...
    public class PhysicsSystem
    {
        ...
        private bool ProcessCollision(CollisionInfo collision, float dt, bool firstContact)
        {
            ...
            for (int pos = 0; pos < collision.PointInfo.Count; ++pos)
            {
                ...
                {
                    Vector3 v0, v1;
                    body0.GetVelocity(ref ptInfo.R0, out v0);
                    body1.GetVelocity(ref ptInfo.R1, out v1);
                ...
                body0.ApplyBodyWorldImpulse(ref impulse, ref ptInfo.R0);

                if (body1 != null)
                    body1.ApplyNegativeBodyWorldImpulse(ref impulse, ref ptInfo.R1);
                ...
                Vector3 vrNew;
                body0.GetVelocity(ref ptInfo.R0, out vrNew);
                ...
                if (tangentSpeed > minVelForProcessing)
                {
                    ...
                    if (denominator > JiggleMath.Epsilon)
                    {
                        ...
                        T *= frictionImpulse;
                        body0.ApplyBodyWorldImpulse(ref T, ref ptInfo.R0);
                        if (body1 != null)
                            body1.ApplyNegativeBodyWorldImpulse(ref T, ref ptInfo.R1);
        ...
        private bool ProcessCollisionFast(CollisionInfo collision, float dt, bool firstContact)
        {
            ...
            bool gotOne = false;
            for (int pos = collision.PointInfo.Count; pos-- != 0; )
            {
                CollPointInfo ptInfo = collision.PointInfo[pos];
                ...
                // recalculate the velocity since it's changed.
                Vector3 vrNew = body0.GetVelocity(ptInfo.R0);
                if (body1 != null)
                    vrNew -= body1.GetVelocity(ptInfo.R1);
                ...
                if (tangentSpeed > minVelForProcessing)
                {
                    ...
                    if (!body0.Immovable)
                    {
                        ...
                        Vector3 v1; float f2;
                        Vector3.Cross(ref ptInfo.R0, ref T, out v1);
                        Vector3.Transform(ref v1, ref body0.worldInvInertia, out v1);
                        Vector3.Cross(ref v1, ref ptInfo.R0, out v1);
                    ...
                    if ((body1 != null) && (!body1.Immovable))
                    {
                        ...
                        Vector3 v1; float f2;
                        Vector3.Cross(ref ptInfo.R1, ref T, out v1);
                        Vector3.Transform(ref v1, ref body1.worldInvInertia, out v1);
                        Vector3.Cross(ref v1, ref ptInfo.R1, out v1);
                        Vector3.Dot(ref T, ref v1, out f2);
        ...
        private bool ProcessCollisionAccumulated(CollisionInfo collision, float dt, bool firstContact)
        {
            ...
            for (int pos = collision.PointInfo.Count; pos-- != 0; )
            {
                ...
                if (doCorrection)
                {
                    ...
                    {
                        Vector3 v0, v1;
                        body0.GetVelocityAux(ref ptInfo.R0, out v0);
                        body1.GetVelocityAux(ref ptInfo.R1, out v1);
                        Vector3.Subtract(ref v0, ref v1, out v0);
                        Vector3.Dot(ref v0, ref N, out normalVel);
                        //normalVel = Vector3.Dot(v0, N);
                    ...
                    if (System.Math.Abs(deltaVel) > minVelForProcessing)
                    {
                        ...
                        body0.ApplyBodyWorldImpulseAux(ref impulse, ref ptInfo.R0);
                        if (body1 != null)
                            body1.ApplyNegativeBodyWorldImpulseAux(ref impulse,ref ptInfo.R1);
                        //prepare our return value
                        gotOne = true;
                ...
                if (ptInfo.AccumulatedNormalImpulse > 0.0f)
                {
                    ...
                    {
                        Vector3 pt1Vel;
                        body1.GetVelocity(ref ptInfo.R1, out pt1Vel);
                    ...
                    if (tangentSpeed > minVelForProcessing)
                    {
                        ...
                        if (denominator > JiggleMath.Epsilon)
                        {
                            ...
                            body0.ApplyBodyWorldImpulse(ref actualFrictionImpulse, ref ptInfo.R0);
                            if (body1 != null)
                                body1.ApplyNegativeBodyWorldImpulse(ref actualFrictionImpulse, ref ptInfo.R1);
        ...
        private unsafe bool ProcessCollisionCombined(CollisionInfo collision, float dt, bool firstContact)
        {
            ...
            for (pos = collision.PointInfo.Count; pos-- != 0; )
            {
                CollPointInfo ptInfo = collision.PointInfo[pos];
                impulses[pos] = 0.0f;
        ...
        private bool ProcessCollisionsForShock(CollisionInfo collision, float dt)
        {
            ...
            for (pos = 0; pos < collision.PointInfo.Count; ++pos)
            {
                CollPointInfo ptInfo = collision.PointInfo[pos];
            }
            ...
            for (int iteration = 0; iteration < iterations; ++iteration)
            {
                for (pos = 0; pos < collision.PointInfo.Count; ++pos)
                {
                    ...
                    if (body0 != null)
                        body0.ApplyBodyWorldImpulseAux(ref actualImpulse,ref ptInfo.R0);
                    if (body1 != null)
                        body1.ApplyNegativeBodyWorldImpulseAux(ref actualImpulse,ref ptInfo.R1);
        ...
        private void PreProcessCollisionFast(CollisionInfo collision, float dt)
        {
            ...
            if (collision.PointInfo.Count > keep)
            {
                collision.PointInfo.Sort(MoreCollPtPenetration);
                collision.PointInfo.RemoveRange(keep, collision.PointInfo.Count - keep);
            }
            ...
            for (int pos = 0; pos < collision.PointInfo.Count; ++pos)
            {
                ...
                else
                {
                    ...
                    Vector3.Cross(ref ptInfo.R0, ref N, out cross);
                    Vector3.Transform(ref cross, ref body0.worldInvInertia, out cross);
                    Vector3.Cross(ref cross, ref ptInfo.R0, out cross);
                ...
                if ((body1 != null) && !body1.Immovable)
                {
                    ...
                    Vector3 cross; float res;
                    Vector3.Cross(ref ptInfo.R1, ref N, out cross);
                    Vector3.Transform(ref cross, ref body1.worldInvInertia, out cross);
                    Vector3.Cross(ref cross, ref ptInfo.R1, out cross);
                    Vector3.Dot(ref N, ref cross, out res);
        ...
        private void BuildIslands()
        {
            ...
            foreach (Body body in bodies)
            {
                if (!body.foundIsland)
                {
                    CollisionIsland island = new CollisionIsland();
        ...
        private void UpdateContactCache()
        {
            ...
            for (int i = collisions.Count; i-- != 0; )
            {
                ...
                for (int pos = 0; pos < collInfo.PointInfo.Count; ++pos)
                {
                    ...
                    contact.Pair = new Contact.BodyPair(collInfo.SkinInfo.Skin0.Owner, collInfo.SkinInfo.Skin1.Owner, ref ptInfo.R0, ref ptInfo.R1);
